...
class SmoothModelIteratorTest extends TestCase
{
    ...
    public function testCurrent()
    {
        ...
        $this->assertEquals(1, $current->modelid);
        $this->assertEquals('#subject', $current->subject);
        $this->assertEquals('#predicate', $current->predicate);
        $this->assertEquals('obb', $current->object);
        $this->assertEquals(898, $current->id);
        $this->assertEquals('en-US', $current->lg);
    ...
    public function testNext()
    {
        ...
        $this->assertInstanceOf('core_kernel_classes_Triple', $current);
        $this->assertEquals(1, $current->modelid);
        $this->assertEquals('#subject2', $current->subject);
        $this->assertEquals('#predicate2', $current->predicate);
        $this->assertEquals('ob2', $current->object);
        $this->assertEquals(899, $current->id);
        $this->assertEquals('en-US', $current->lg);
